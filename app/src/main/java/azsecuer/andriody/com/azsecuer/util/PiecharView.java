...
public class PiecharView extends View {
	...
	public PiecharView(Context context, AttributeSet attrs) {
		...
		//
		bgColor = context.getResources().getColor(R.color.piechar_phonespace_progressbar_bg);
		//
		datas = new int[0][0];
		Logutil.p("CustomView","构造方法-paint"+paint);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Logutil.p("CustomView","onMeasure-w"+w+"-h"+h);
		piecharRect = new RectF(0, 0, w, h); // 确定饼形圆环大小
		Logutil.p("CustomView","onMeasure-Rect"+piecharRect);
	...
	protected void onDraw(Canvas canvas) {
		...
		Logutil.p("CustomView","OnDraw()"+count++);
	}
}
